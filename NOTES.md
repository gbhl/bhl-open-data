...
There is a mixed relationship between Items and Parts. OCR files must be accessed in different ways depending on the data.
...
Some Parts do not have parent items. If a part has a `BarCode` in `part.txt` then it will have a correspoding OCR file prefixed with `part`. There is no corresponding OCR for the Item. In `item.txt` these will have a BarCode that looks like `vi210914v100201120250316010124` (regex `/^vi\d{6}v/`)
